...
The real-browser editor regression also requires an installed Chromium (`CHROMIUM_BIN=/path/to/chromium` overrides discovery). On this ARM64 workspace Chromium is at `/usr/local/chromium/bin/chrome`; its missing Debian `libnss3`/`libnspr4` libraries were extracted into the disposable `.verify-runtime/browser-libs/` cache, without system installation. The runner recognizes that cache when present. Other hosts should supply a working Chromium installation. Browser tests fail rather than skip if unavailable. Chromium runs a repo-owned offline HTML fixture with a temporary profile, no credentials and hostname resolution disabled. Restricted execution environments may need permission to launch browser processes.
